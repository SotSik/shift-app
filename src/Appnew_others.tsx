...
export default function Appnew_others(user){
  const [displayedUser, setUser] = useState("");
  ...
  const [visible , setClose] = useState(false);
...
if (isLoading) {
  ...
  return (<div  style = {{backgroundColor : "#ffffff"}}> <MemberShiftData value = {memberShifts}><AllMember value = {gotMember}><AllShift value = {gotallShift}>
  <div style = {{height:"85px",  display: "flex",justifyContent: "flex-end"}} ><Button as = "a" 
  component="label"
  variant="contained"
  ...
  <div className = {"container"}><DisplayShifts shifts = {memberShifts[displayedUser]}  members = {gotMember}/></div></AllShift></AllMember></MemberShiftData>
  </div>
  );
}
...
function DisplayShifts({shifts,members} : {shifts : object[],members : string[]}){
  ...
  if(shifts) {
    ...
    if(!dayList.includes(strDate(s.start))&& today > strDate(s.start)){
      ...
      return(
      <div style = {{alignItems : "center"}}><h2>{strDate(s.start)}</h2>
...
function Person({member,v,setOthers,s}  : {member:string[],v:string,setOthers:any,s:ShiftItem}){
  ...
  if(v != user) return (<span><Button variant="outlined" onClick = {handleClick} sx = {{textTransform: 'none'} }style={{ zIndex:10000 }}
  ...
  return  (<span><Button variant="outlined" onClick = {handleClick} sx = {{textTransform: 'none'}} style={{ zIndex: 10000 }}
  >{v}</Button><Popper id={id} open={open} anchorEl={anchorEl} popperOptions = {{strategy : "fixed"}}>
    <Box sx = {{ border: 1, p: 1, bgcolor : "background.paper" , zIndex: 999999}}>
